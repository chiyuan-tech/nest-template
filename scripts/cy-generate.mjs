...
import { manifest, validateCyTemplate } from "./cy-schema.mjs";
...
/** CY 画廊 PricingCase* 不进入首页；由公共 PricingSection 承接（见 shared-ui-components skill） */
const CY_PRICING_CASE_KEY = /^PricingCase\d+$/;
const CY_FAQ_CASE_KEY = /^FaqCase\d+$/;

function isSharedPricingInjectionPoint(templateKey) {
...
function buildPageStackLines(blocks) {
  ...
  return { stackLines, sharedPricingInserted };
}

function buildMultiBlockPageSource({ blocks, regenerateHint }) {
  const pageBlocks = blocks.filter((b) => !CY_PRICING_CASE_KEY.test(b.templateKey));
  const importLines = buildModuleImportLines(pageBlocks);
  const { stackLines, sharedPricingInserted } = buildPageStackLines(blocks);
  const pricingImport = sharedPricingInserted
    ? `import PricingSection from "@/components/PricingSection";\n`
    : "";
  const stack = stackLines.join("\n");
  return `/** Generated by \`${regenerateHint}\` — source: cy_templeate.json. Re-run to refresh. */
import { CySaasPageShell } from "@/components/cy/layout/CySaasPageShell";
${pricingImport}${importLines}
...
function listTemplateBlocksForPage(template) {
  ...
  for (const templateKey of keys) {
    ...
    const registered = manifest.modules[templateKey];
    if (!registered) throw new Error(`[cy-generate] Unregistered CY module key: ${templateKey}`);
    if (normalizeImportPath(hint) !== registered.componentPath) throw new Error(`[cy-generate] ${templateKey} path must match the CY registry.`);
    blocks.push({
      templateKey,
      componentName: templateKey,
      importSpec: registered.componentPath,
    });
  }
  if (blocks.length > 1) {
    console.log(
...
function main() {
  ...
  const template = validateCyTemplate(JSON.parse(fs.readFileSync(TEMPLATE_PATH, "utf8")));
